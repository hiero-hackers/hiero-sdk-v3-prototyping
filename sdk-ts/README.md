# Hiero SDK V3 (JS Implementation)

This repository contains the prototyping ground and active implementation for the **next generation of client SDKs ("V3")** for the [Hiero](https://hiero.org) project under Linux Foundation Decentralized Trust (LFDT).

## 🚀 Local Development Setup

To get started with compiling, testing, and developing the V3 JS SDK locally, follow these steps:

### 1. Prerequisites
- **Node.js**: v20 or higher is recommended.
- **Git**: Required to pull the protobuf definitions submodule.

### 2. Install Dependencies
Clone the repository and install the standard dependencies:
```bash
git clone https://github.com/hiero-ledger/hiero-sdk-v3-js.git
cd hiero-sdk-v3-js
npm install
```

### 3. Initialize Protobufs
The V3 SDK securely abstracts network payloads utilizing static Protobuf code generation from the `hiero-consensus-node` repository.

There are two ways to download the submodule:
**Option A: Clone with submodules automatically**
```bash
git clone --recursive https://github.com/hiero-ledger/hiero-sdk-v3-js.git
```
**Option B: Initialize manually (if already cloned)**
```bash
git submodule update --init --recursive
```
*(Note: If you need to add the submodule from scratch or update its remote URL, use: `git submodule add https://github.com/hiero-ledger/hiero-consensus-node.git src/base/internal/submodules/hiero-consensus-node`)*

Once the submodule is downloaded, flatten, deduplicate, and compile the `.proto` files into JS/TS bindings:
```bash
node scripts/prepare-protos.js
npm run build:proto
```
*(This will generate `src/base/internal/generated-proto.js` and `.d.ts` used by `src/base/proto.ts`)*

### 4. Running Tests
The test suite validates the specific V3 implementations against the core specification:
```bash
...
### 5. Type Checking
Ensure all implementations strictly adhere to TypeScript's `verbatimModuleSyntax` and types:
```bash
npx tsc --noEmit
```
...
*(The original meta-specification details continue below)*
