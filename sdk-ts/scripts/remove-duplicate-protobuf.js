...
const PROTO_PACKAGE_ROOT = path.join(path.dirname(SCRIPT_DIR), "src", "base", "internal");
const ROOT = path.join(PROTO_PACKAGE_ROOT, 'proto-src');
...
for (const file of stateSignatureFiles) {
    if (file !== canonicalFile && fs.existsSync(canonicalFile)) {
